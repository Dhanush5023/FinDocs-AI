...
# ---------------------------------------------------------
# Page Configuration & Ultra-Premium Styling
# ---------------------------------------------------------
st.set_page_config(
            ...
            tables=[
                ExtractedTable(
                    ...
                    rows=[
                        ...
                        ["Cloud Hosting (AWS us-east-1 Core)", "$820,000", "$940,000", "+14.6%", "Over Budget ? Requires Optimization"],
                        ["Engineering Personnel & Equity Comp", "$2,100,000", "$2,350,000", "+11.9%", "On Track with Hiring Plan"],
...
with st.sidebar:
    ...
    with col_s4:
        st.markdown(f'<div class="glass-card"><div class="metric-title">Cost Saved</div><div class="metric-value">${summary.get("estimated_savings_vs_naive_usd", 0.0)}</div><div class="metric-sub">vs Naive</div></div>', unsafe_allow_html=True)
...
with tab_query:
    ...
    with col_q_left:
        ...
        if (exec_btn or q_pick) and user_query.strip():
            ...
            if cached_result:
                ...
                st.markdown(f'<div style="background:rgba(16,185,129,0.12);border:1px solid rgba(16,185,129,0.3);border-radius:10px;padding:10px 14px;font-size:0.82rem;color:#34D399;font-family:JetBrains Mono,monospace;margin-bottom:14px;">? <strong>LRU Cache Hit:</strong> Served from in-memory cache in <strong>{lat:.2f} ms</strong> (0 Tokens Consumed, $0.00 Cost).</div>', unsafe_allow_html=True)
    ...
    with col_q_right:
        ...
        st.markdown("##### ??? Production Guardrails")
...
with tab_roi:
    ...
    with col_r2:
        ...
        st.markdown(f'<div class="glass-card" style="margin-top:16px;border-color:rgba(16,185,129,0.4);"><div style="font-size:0.9rem;font-weight:700;color:#34D399;margin-bottom:6px;">?? Projected Annual ROI: ${annual_savings:,.2f} / Year</div><div style="font-size:0.8rem;color:#94A3B8;line-height:1.5;">By eliminating full-document token ingestion and serving repeat queries from the LRU cache with sub-millisecond latency, your team saves <strong>${annual_savings:,.2f} annually</strong> while maintaining 100% precision on numerical balance sheet data.</div></div>', unsafe_allow_html=True)
